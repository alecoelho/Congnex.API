...
public class XylaController : ControllerBase
{
    ...
    /// <summary>
    /// Verifica se o aluno está pronto para iniciar as lições após a entrevista.
    /// Retorna ready=true assim que a entrevista for salva e houver lições globais no banco.
    /// </summary>
    [HttpGet("ready")]
    public async Task<ActionResult> Ready(CancellationToken ct)
    {
        ...
        // Lições globais disponíveis no banco?
        var globalLessonCount = await db.Lessons
            .CountAsync(l => l.UserId == null, ct);

        // Primeira unidade com lições globais
        var unit = await db.Units
            .Where(u => u.Lessons.Any(l => l.UserId == null))
        ...
        return Ok(new
        {
            ready       = globalLessonCount > 0,
            lessonCount = globalLessonCount,
