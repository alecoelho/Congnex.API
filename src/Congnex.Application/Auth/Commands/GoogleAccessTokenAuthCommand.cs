...
public record GoogleAccessTokenAuthCommand(string AccessToken) : IRequest<AuthResult>;

public sealed class GoogleAccessTokenAuthCommandHandler(
    ICongnexDbContext db,
    IJwtTokenService jwt,
    IPasswordHasher hasher,
    IGoogleAuthService google,
    IEmailService email,
    IOptions<JwtSettings> jwtOpts) : IRequestHandler<GoogleAccessTokenAuthCommand, AuthResult>
{
    private readonly JwtSettings _jwt = jwtOpts.Value;

    public async Task<AuthResult> Handle(GoogleAccessTokenAuthCommand req, CancellationToken ct)
    {
        var info = await google.GetUserInfoFromAccessTokenAsync(req.AccessToken)
