...
public sealed class GoogleAuthCommandHandler(
    ...
    IEmailService email,
    IOptions<JwtSettings> jwtOpts) : IRequestHandler<GoogleAuthCommand, AuthResult>
{
    private readonly JwtSettings _jwt = jwtOpts.Value;

    public async Task<AuthResult> Handle(GoogleAuthCommand req, CancellationToken ct)
    {
        var info = await google.VerifyIdTokenAsync(req.IdToken)
