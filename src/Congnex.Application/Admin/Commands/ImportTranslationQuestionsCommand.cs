...
public record ImportTranslationQuestionsCommand(Guid LessonId, List<TranslationRowDto> Rows)
    : IRequest<ImportQuestionsResult>;

public sealed class ImportTranslationQuestionsHandler(ICongnexDbContext db)
    : IRequestHandler<ImportTranslationQuestionsCommand, ImportQuestionsResult>
{
    public async Task<ImportQuestionsResult> Handle(
        ImportTranslationQuestionsCommand cmd, CancellationToken ct)
    {
